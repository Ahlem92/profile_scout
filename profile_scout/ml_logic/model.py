...
def apply_cosine (data:pd.DataFrame, player = str):
    v1 = np.array(data.loc[player]).reshape(1, -1)
...
def merge_cosinedf (X:pd.DataFrame,df:pd.DataFrame,player= str,number=int):
    results=pd.merge(X ,df,left_index=True, right_index=True).sort_values(by="Similar Players",ascending=False)
    bp=results.loc[player]["Best Position"]
